...
//checking if admin or user
    ...
    if(role==='user'){
        ...
        if(!user || user === null){
            ...
            return res.status(401).render("auth/signin",{messages:"Invalid Username or Password"})
         }
          ...
          if(match){
             noerror=true
                 ...
                 if (noerror) {
                     ...
                     res.status(401).render("auth/signin",{messages:"Invalid Username or Password"})
                 }
    }else{
        if(!user || user === null){
            return res.status(401).render("auth/adminSignin",{messages:"Invalid Username or Password"})
         }
